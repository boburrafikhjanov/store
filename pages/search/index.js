...
import SearchResult from "../../components/SeatchResult";

import { getAsString } from "../../helpers/arrayOperations";
import Tracker from '../../components/Tracker/index'

const SearchPage = () => {
  const { searchProducts } = useTypeSelector((state) => state.search);
  const { getSearchResults } = useTypeDispatch();
  ...
  return (
    ...
    <Head>
      ...
      <Tracker/>
      <SearchResult data={searchProducts} query={query.param} />
    </>
  );
};
